<template>
  <div>
    <div class="option">
      <el-button type="primary" icon="Plus" @click="handleAdd">新增Secrets</el-button>
    </div>
    <el-card>
...
import { getSecrets, saveSecret, updateSecret, delSecret } from '~/api/client/secrets'
...
const state = reactive({
  ...
  rules: {
    ...
    secret: [
      { required: true, message: 'secret 不能为空', trigger: 'blur' }
    ]
  }
})
...
function getList() {
  state.loading = true
  getSecrets(clientId).then((res:any) => {
    state.dataList = res
  }).finally(() => {
    state.loading = false
  })
...
function submitForm() {
  formRef.value.validate((valid: boolean) => {
    if (valid) {
      ...
      let { id, name, secret } = state.form

      const params = { name, secret }

      if (state.open === Status.EDIT) {
        updateSecret(clientId, id as number, params).then(() => {
          ElMessage({
            showClose: true,
            message: '修改成功',
            type: 'success',
        ...
        saveSecret(clientId, params).then(() => {
          ElMessage({
            showClose: true,
            message: '创建成功',
            type: 'success',
...
function handleDelete(row: any) {
  ElMessageBox.confirm(
    ...
    await delSecret(clientId, row.id)
