...
import { getUsers, saveUser, updateUser, delUser } from '~~/api/client/resource-type/roles/roles-user'
import { getClientUsers } from '~/api/common'

const route = useRoute()
const { clientId: clientId, type, resource, roleName } = route.params
...
function getList() {
  state.loading = true
  getUsers(clientId, type, resource, roleName).then((res:any) => {
    state.dataList = res
  }).finally(() => {
    state.loading = false
  })
...
// 获取client列表
async function getClientUsersListList() {
  const clientUsersList: any = await getClientUsers(clientId)
  cilentUsersOption.value = clientUsersList.map((item: any) => ({
    label: item.name,
    value: item.id
  }))
}
  ...
  formRef.value.validate((valid: boolean) => {
    if (valid) {
      ...
      if (state.open === Status.EDIT) {
        updateUser(clientId, id as number,type, resource, roleName,  params).then(() => {
          ElMessage({
            showClose: true,
            message: '修改成功',
            type: 'success',
        ...
        saveUser(clientId, type, resource, roleName, params).then(() => {
          ElMessage({
            showClose: true,
            message: '创建成功',
            type: 'success',
...
function handleDelete(row: any) {
  ElMessageBox.confirm(
    ...
    await delUser(clientId, type, resource, roleName, row.name)
