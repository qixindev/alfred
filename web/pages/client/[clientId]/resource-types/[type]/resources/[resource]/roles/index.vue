<template>
  <div>
    <div class="option">
      <el-button type="primary" icon="Plus" @click="handleAdd">新增Roles</el-button>
    </div>
    <el-card>
      <el-table v-loading="loading" stripe :data="dataList">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="{ row }">
            <el-button size="small" type="primary" link icon="Edit" @click="viewUsers(row)">user管理
            </el-button>
  ...
  formRef.value.validate((valid: boolean) => {
    if (valid) {
      ...
      if (state.open === Status.EDIT) {
        updateRole(clientId, id as number,type,  params).then(() => {
          ElMessage({
            showClose: true,
            message: '修改成功',
            type: 'success',
        ...
        saveRole(clientId, type, params).then(() => {
          ElMessage({
            showClose: true,
            message: '创建成功',
            type: 'success',
...
function handleDelete(row: any) {
  ElMessageBox.confirm(
    ...
    await delRole(clientId, type, row.name)
...
function viewUsers(row: any) {
  navigateTo(`/client/${clientId}/resource-types/${type}/resources/${resource}/roles/${row.name}/user`)
}
