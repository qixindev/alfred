...
import { getGroups, saveGroup, updateGroup, delGroup } from '~/api/devices/groups'
...
function getList() {
  state.loading = true
  getGroups(deviceId).then((res:any) => {
    state.dataList = res
  }).finally(() => {
    state.loading = false
  })
  ...
  formRef.value.validate((valid: boolean) => {
    if (valid) {
      ...
      if (state.open === Status.EDIT) {
        updateGroup(deviceId, id as number, params).then(() => {
          ElMessage({
            showClose: true,
            message: '修改成功',
            type: 'success',
        ...
        saveGroup(deviceId, params).then(() => {
          ElMessage({
            showClose: true,
            message: '创建成功',
            type: 'success',
