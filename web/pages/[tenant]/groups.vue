<template>
  <div>
    <div class="option">
      <el-button type="primary" icon="Plus" @click="handleAdd">新增Groups</el-button>
    </div>
    <el-card>
...
import { getGroups, saveGroup, updateGroup, delGroup } from '~/api/groups'
  ...
  formRef.value.validate((valid: boolean) => {
    if (valid) {
      ...
      if (state.open === Status.EDIT) {
        updateGroup(id as number, params).then(() => {
          ElMessage({
            showClose: true,
            message: '修改成功',
            type: 'success',
